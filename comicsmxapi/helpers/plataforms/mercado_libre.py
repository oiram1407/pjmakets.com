...
class MercadoLibreHelper:
    ...
    def setInventory(self, new_inventory):
        variations = list(self.getProductDataByID(new_inventory))
    ...
    def calcShipping(self, data):
        ...
        _url = f"{self._api_meli}/users/{self._user_id}/shipping_options/free?dimensions={data['product_height']}x{data['product_width']}x{data['product_length']},{_weight}&verbose=TRUE&item_price={data['product_price']}&listing_type_id={data['product_listing_type_id']}&mode=me2&condition=new&logistic_type=xd_drop_off"
    ...
    def serarchISBN(self, title, serie, editorial):
        ...
        for i in range(len((_items))):
            ...
            _book_title = _book_info['title'].lower()
            if(serie in _book_title):
                author = _book_info['authors'][0]
    ...
    def setNewProduct(self, product_data):
        ...
        isbn = isbn if(isbn) else product_data['product_isbn']
    ...
    def setProductUpdate(self, product_data):
        ...
        if(response['status'] == 400):
            ...
            response = self.setInventory(product_data['product_stock'])
