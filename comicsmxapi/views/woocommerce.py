...
class WoocommerceWebhook(APIView):
    ...
    def checkOrder(self):
        #order = OrderPlataformRelated.objects.get(order_plataform_id=self._order_id, company=self._company.id)
        order = None
        return order
    ...
    def post(self, request):
        ...
        if type_event == 'order':
            ...
            if action_event == 'created':
                ...
                if order_data['status'] == 'paid':
                    ...
                    if(shp_lines['method_id'] == 'local_pickup'):
                        ...
                        delivery_location = "la rock show en metro hidalgo"
                        delivery_instruction = "nos encontramos frente al local el kioskito"
                        delivery_schedule = "11:00am a 4:00pm"
                    ...
                    for product in products_list:
                        ...
                        if(_mercado_libre_product_id is None):
                            ...
                            _mercado_libre_product = _mercado_libre.setInventory(new_inventory)
            ...
            editorials = ("Panini Manga", "Kamite", "MangaLine", "Planeta Comics", "DC", "Marvel", "Norma", "Ivrea", "Milky Way Ediciones")
            ...
            if action_event == 'created':
                ...
                if (self._product is None):
                    ...
                    data_response['id'] = f"The product already exists with de the next ID {self._product.id}"
                ...
                if (product_obj.getProductBySKU() is None):
            ...
            _check_meli_product = product_obj_meli.getIDProductBySKU(product_sku)
            ...
            if(_check_meli_product is None):
                ...
                _data_product_price_meli = {
                        ...
                        "product_price":  product_obj_meli_new['product_price'],
                        "user": self._company.user.id
                    }

                _product_price_meli = product_obj.setProductPrice(_data_product_price_meli)
                data_response['meli_id'] = f"The product has been created succesfully in MELI with ID {_meli_product.id}"
            
            else:
                self.check_product_mercado_libre = product_obj.getProductRelatedID(self._product.id, "mercado_libre")
                ...
                if(self.check_product_mercado_libre is None):
                    ...
                    _product_related_meli = product_obj.setProductRelated(data_meli_order)
                    
                    data_response['relaed_meli_order'] = f"The meli product {_check_meli_product} has been created with ID {_product_related_meli.id}"
